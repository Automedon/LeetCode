...
var Twitter = function () {
  this.users = new Map(); // user->usersFollowers
  this.tweets = []; //All tweets of the tweeter
};
...
Twitter.prototype.getNewsFeed = function (userId) {
  ...
  for (let i = this.tweets.length - 1; i >= 0 && recentTweets.length < 10; i--) {
    if (this.users.get(userId).has(this.tweets[i].userId) || this.tweets[i].userId === userId) {
